...
# class labels convert to int
target = tpot_data['stress'].astype('int')

features = np.delete(tpot_data.view(np.float64).reshape(tpot_data.size, -1), tpot_data.dtype.names.index('stress'),
                     axis=1)
...
# weights parameter 'distance'-> 'uniform' since 'distance' is not supported for CoreML conversion
exported_pipeline = KNeighborsClassifier(n_neighbors=4, weights="uniform")
